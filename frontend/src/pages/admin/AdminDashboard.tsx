    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { icon: TrendingUp, label: 'Investments', value: stats?.totalInvestments || 0, color: 'text-primary-400', bg: 'bg-primary-400/10' },
          { icon: DollarSign, label: 'Revenue', value: `$${stats?.totalRevenue || 0}`, color: 'text-green-400', bg: 'bg-green-400/10' },
          { icon: Package, label: 'Products', value: stats?.totalProducts || 0, color: 'text-purple-400', bg: 'bg-purple-400/10' },
          { icon: Clock, label: 'Pending Payments', value: stats?.payments.pending || 0, color: 'text-yellow-400', bg: 'bg-yellow-400/10' },
          { icon: CheckCircle, label: 'Completed', value: stats?.payments.finished || 0, color: 'text-green-400', bg: 'bg-green-400/10' },
          { icon: MessageSquare, label: 'Open Tickets', value: stats?.openTickets || 0, color: 'text-red-400', bg: 'bg-red-400/10' },
          { icon: TrendingUp, label: 'This Month', value: stats?.investmentsThisMonth || 0, color: 'text-primary-400', bg: 'bg-primary-400/10' },
        ].map((stat, i) => (
          <motion.div
            key={i}
            className="card"
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { to: '/admin/tracking', label: 'View Analytics', icon: TrendingUp },
        ].map((link, i) => (
          <Link
            key={i}
            to={link.to}
            ...
            <tbody>
              {recentInvestments.slice(0, 5).map((inv) => (
                <tr key={inv.id} className="border-b border-primary-400/5">
                  ...
                  <td className="py-3 text-gray-400">{inv.productionStatus?.replace(/_/g, ' ')}</td>
                </tr>
              ))}
            </tbody>
          </table>
